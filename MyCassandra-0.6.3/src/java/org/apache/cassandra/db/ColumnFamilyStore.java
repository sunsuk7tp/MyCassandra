...
public class ColumnFamilyStore implements ColumnFamilyStoreMBean
{
    ...
    Memtable apply(DecoratedKey key, ColumnFamily columnFamily)
    {
        ...
        try {
        	try {
				if(dbi.insertOrUpdate(key.toString(), columnFamily) < 0)
...
// TODO this actually isn't a good meature of pending tasks
    ...
    public ColumnFamily getColumnFamily(QueryFilter filter, int gcBefore)
    {
        ...
        try
        {
        	...
        	{
        		return dbi.select(filter.key.toString(), filter);
        	}
        	ColumnFamily cf = dbi.select(filter.key.toString(), filter);
        	IColumnIterator ci = filter.getMemtableColumnIterator(cf, null, getComparator());
