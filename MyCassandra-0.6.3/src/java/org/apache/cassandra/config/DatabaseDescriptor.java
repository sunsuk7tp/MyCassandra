...
public class DatabaseDescriptor
{
    ...
    static
    {
        try
        {
            ...
            /* sql configure */
            sqlHost = conf.sqlhost;
            sqlPort = conf.sqlport;
            sqlUser = conf.sqluser;
            sqlPass = conf.sqlpass;
    ...
    public static Collection<KSMetaData> readTablesFromYaml() throws ConfigurationException
    {
        ...
        for (Keyspace keyspace : conf.keyspaces)
        {
            ...
            for (ColumnFamily cf : keyspace.column_families)
            {
                ...
                if (cf.row_key_size > 0)
                {
                	rowKeySize = cf.row_key_size;
                }
                if (cf.column_family_size > 0)
                {
                	columnFamilySize = cf.column_family_size;
                }
                if (cf.column_family_type != null)
                {
                	columnFamilyType = cf.column_family_type;
                }
                if (cf.storage_engine_type != null)
                {
                	storageEngineType = cf.storage_engine_type;
                }
                
                if(dataBase == MYSQL) {
                	// make sql table
                	MySQLInstance dbi = new MySQLInstance(keyspace.name, cf.name);
                	try {
                		dbi.create(cf.name, rowKeySize, columnFamilySize, columnFamilyType, storageEngineType);
